...
@MessageDriven(messageListenerInterface = MessageListener.class, mappedName = "jms/createUserFinalizeSuccessQueue", activationConfig = { @ActivationConfigProperty(propertyName = "destinationType", propertyValue = "javax.jms.Queue") })
@TransactionAttribute(TransactionAttributeType.MANDATORY)
public class CreateUserFinalizeSuccessHandler {
	...
	public void onMessage(Message message) {
		System.out.println("CreateUserFinalizeSuccessHandler");
		if (message instanceof ObjectMessage) {
			try {
				Serializable object = ((ObjectMessage) message).getObject();
				if (object instanceof CreateUserSuccess) {
					CreateUserSuccess data = (CreateUserSuccess) object;
					CreateUserDataResponse response = UserExtDomainObjectFactory.getInstance()
							.createCreateUserDataResponse(0, null, data.getUserData().getId(),
									data.getAsyncResponseId());
					send(response);
				} else {
